...
1. Traits - These are also used for objects and rooms. They define the
            base power and attribuites that describe an object. How strong is
...
2. Mutations - In DOG, a virus has infected animal and plant life which causes
               ...
               flight or psionic abilities. Most mutations are applied at the
               object part level. For example, a character may grow scales on
               their left arm as a mutation. Maybe that mutation later spreads
               to other body parts. Mutations are passive; they don't control
               actions, but they may modify or enable actions. Actions are
               controlled by Talents.
...
4. Status Effects - Status Effects are temporary changes to the state of an
            object (or a part of an object that is itself an object) that can
            be added, removed, or changed. Examples of status effects are wet,
            rusting, fatigued, wounded (with a number of sub-effects), high,
            drunk, etc. These status effects typically have a numerical value
            associated to them to indicate the strength of the effect, and some
            indicators of how the effect can change over time (healed). Some
            status effects have a real impact on gameplay, but others are just
            cosmetic.
Traits Module
    ...
    def percent_bar(self):
        ...
        if self._type in RANGE_TRAITS:
            if self.current * 100.0 / self.max >= 95:
                ...
                return "[|R░                |n]"
            elif self.current * 100.0 / self.max <= 1:
                return "[|R                 |n]"
        # if we get to this point, it's either a static trait or
        # a divide by zero situation
        return "[|R                 |n]"
